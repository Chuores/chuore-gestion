...
export default function InicioPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        const y26 = d.getFullYear()
        const m26 = String(d.getMonth() + 1).padStart(2, '0')
        const pre26 = y26 + '-' + m26
        const pre25 = (y26 - 1) + '-' + m26
  ...
  const mesEquiv25 = '2025-' + today.slice(5, 7)
  ...
  const ventasMes25 = Object.entries(d25).filter(([k]) => k.startsWith(mesEquiv25 + '-') && parseInt(k.slice(8)) <= diaActual).reduce((s, [, v]) => s + v, 0)
  ...
  return (
    <div className="fade-in" style={{ maxWidth: '960px' }}>
        ...
        <div style={{ borderRadius: '10px', padding: '14px 18px', marginBottom: '20px', border: '2px solid ' + (posReal ? 'var(--green-border)' : 'var(--c-red-border)'), background: posReal ? 'var(--green-bg)' : 'var(--c-red-bg)', display: 'flex', gap: '12px', alignItems: 'center' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '12px', color: 'var(--text-2)' }}>
              {euro(mediaYtd26)}/día en 2026 vs {euro(mediaYtd25)}/día en 2025.{' '}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '10px', marginBottom: '14px' }}>
        {[
          { label: mesNombre + ' 2025 (hasta d' + diaActual + ')', value: loading ? '—' : euro(ventasMes25), sub: diasMes25 + ' días · ' + euro(mediaMes25) + '/día', color: 'var(--text-3)', border: 'var(--border)' },
          { label: mesNombre + ' 2026', value: loading ? '—' : euro(ventasMes26), sub: diasMes26 + ' días · ' + euro(mediaMes26) + '/día', color: 'var(--red)', border: 'var(--red)' },
          { label: 'Var. mes actual', value: loading ? '—' : sgn(pctMes) + pctMes.toFixed(1) + '%', sub: sgn(r2(mediaMes26 - mediaMes25)) + euro(r2(mediaMes26 - mediaMes25)) + '/día vs 2025', color: posMes ? 'var(--green)' : 'var(--c-red)', border: posMes ? 'var(--green-border)' : 'var(--c-red-border)' },
          { label: 'Crecimiento real YTD', value: loading ? '—' : sgn(pctReal) + pctReal.toFixed(1) + '%', sub: sgn(diffYTD) + euro(diffYTD) + ' vs 2025', color: posReal ? 'var(--green)' : 'var(--c-red)', border: posReal ? 'var(--green-border)' : 'var(--c-red-border)' },
        ].map(k => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '10px', marginBottom: '20px' }}>
        {[
          ...
          { label: 'Media diaria real', value: loading ? '—' : euro(mediaYtd26) + ' vs ' + euro(mediaYtd25), sub: sgn(r2(mediaYtd26-mediaYtd25)) + euro(r2(mediaYtd26-mediaYtd25)) + ' por día', color: posReal ? 'var(--green)' : 'var(--c-red)', border: posReal ? 'var(--green-border)' : 'var(--c-red-border)' },
        ].map(k => (
          <div key={k.label} className="stat-card" style={{ borderTop: '3px solid ' + k.border }}>
            <p className="stat-label">{k.label}</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 260px', gap: '14px' }}>
        <div className="card">
          <div className="card-header">
            <p className="card-title">Ventas mensuales 2025 vs 2026</p>
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', color: 'var(--text-3)' }}><span style={{ width: '10px', height: '4px', background: 'var(--green)', display: 'inline-block', borderRadius: '2px', opacity: 0.8 }} />2026 mejor</span>
              <span style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', color: 'var(--text-3)' }}><span style={{ width: '10px', height: '4px', background: 'var(--c-red)', display: 'inline-block', borderRadius: '2px', opacity: 0.8 }} />2026 peor</span>
            </div>
          </div>
          <div style={{ padding: '20px' }}>
            {loading ? <div style={{ height: '140px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><p style={{ fontSize: '12px', color: 'var(--text-4)' }}>Cargando...</p></div> : (
              <>
                    ...
                    return (
                      <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative', zIndex: 1 }}>
                        <div style={{ width: '100%', display: 'flex', gap: '2px', alignItems: 'flex-end', height: '112px' }}>
                          ...
                          <div style={{ flex: 1, background: isPos ? 'var(--green)' : 'var(--c-red)', borderRadius: '3px 3px 0 0', height: (m.v26 / maxChart * 100) + '%', minHeight: m.v26 > 0 ? '3px' : '0', opacity: 0.8 }} />
                        </div>
                        <div style={{ position: 'absolute', bottom: 0, textAlign: 'center' }}>
                          <p style={{ fontSize: '10px', color: 'var(--text-4)', textTransform: 'capitalize' }}>{m.label}</p>
                          {m.v26 > 0 && m.v25 > 0 && <p style={{ fontSize: '9px', fontWeight: '700', color: isPos ? 'var(--green)' : 'var(--c-red)' }}>{isPos ? '+' : ''}{diff.toFixed(0)}%</p>}
                        </div>
                      </div>
                    )
                  })}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {[
            { href: '/dashboard/comparativa', label: 'Ver comparativa completa', desc: 'Detalle día a día', dark: true },
            { href: '/dashboard/ventas', label: 'Registrar venta', desc: 'Total del TPV de hoy', dark: false },
            { href: '/dashboard/gastos', label: 'Registrar gasto', desc: 'Materias primas, alquiler...', dark: false },
            { href: '/dashboard/pedidos', label: 'Hacer pedido', desc: 'Enviar por WhatsApp', dark: false },
          ].map(item => (
            <Link key={item.href} href={item.href} style={{ textDecoration: 'none' }}>
              <div style={{ background: item.dark ? 'var(--black)' : 'var(--surface)', border: '1px solid ' + (item.dark ? 'transparent' : 'var(--border)'), borderRadius: '10px', padding: '12px 14px', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', transition: 'opacity 0.1s' }}>
                <div>
                  <p style={{ fontSize: '12px', fontWeight: '600', color: item.dark ? 'var(--gold)' : 'var(--text-1)', marginBottom: '2px' }}>{item.label}</p>
                  <p style={{ fontSize: '11px', color: item.dark ? 'rgba(255,255,255,0.3)' : 'var(--text-4)' }}>{item.desc}</p>
                </div>
                <svg width="14" height="14" fill="none" stroke={item.dark ? 'rgba(255,255,255,0.3)' : 'var(--border-2)'} strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7"/></svg>
              </div>
            </Link>
          ))}
        </div>
